const LS_KEY = "toeic_listening_vocab_progress_v1";
...
function partData() {
  return TOEIC_LISTENING_VOCAB.find(p => p.id === currentPart) || TOEIC_LISTENING_VOCAB[0];
}
...
function renderNav() {
  const p = progress();
  document.getElementById("partNav").innerHTML = TOEIC_LISTENING_VOCAB.map(part => {
    ...
    return `<button class="${part.id === currentPart ? "active" : ""}" onclick="selectPart('${part.id}')">
      <strong>${part.part}: ${part.title}</strong>
      <span>${done}/${words.length} tu da hoc</span>
    </button>`;
  }).join("");
}
...
function renderHeader() {
  ...
  document.getElementById("wordCount").textContent = `${words.length} tu trong nhom`;
  document.getElementById("learnedCount").textContent = `${words.filter(w => p[w.word]).length} da danh dau`;
}
...
function renderGrammar() {
  ...
  document.getElementById("modeContent").innerHTML = `
    <div class="grammar-container">
      ...
      <div class="grammar-section-title">🚨 Các bẫy nghe thường gặp (Traps)</div>
  ...
  document.getElementById("modeContent").innerHTML = `
    ...
    <div class="study-card">
      <div>
        ...
        <button class="speak-btn" onclick="speak('${escapeAttr(w.word)}')" title="Nghe phat am">🔊</button>
    ...
    <div class="listening-actions">
      <button class="btn btn-success" onclick="showFlashAnswer()">Hien nghia</button>
      <button class="btn btn-primary" onclick="nextFlash()">Tu tiep theo</button>
      <button class="btn btn-secondary" onclick="prevFlash()">Tu truoc</button>
      <button class="btn btn-warning" onclick="addDifficult('${escapeAttr(w.word)}')">Tu kho</button>
    </div>`;
}
...
function renderQuiz() {
  ...
  if (quizIndex >= quizWords.length) {
    ...
    document.getElementById("modeContent").innerHTML = `<div class="quiz-summary">
      ...
      <div class="quiz-summary-label">Dung ${quizCorrect}/${quizWords.length}</div>
      <button class="btn btn-primary" onclick="startQuiz();renderQuiz()">Lam lai</button>
    </div>`;
    return;
  }
  ...
  document.getElementById("modeContent").innerHTML = `
    ...
    <div class="study-card">
      <div>
        <div class="text-muted">Cau ${quizIndex + 1}/${quizWords.length} · Chon nghia dung</div>
        <div class="study-word">${w.word}</div>
    ...
    <div class="listening-actions"><span class="score-pill">Dung ${quizCorrect}</span></div>`;
}
...
function addDifficult(word) {
  ...
  if (w && typeof Storage !== "undefined") {
    ...
    alert(`Da them "${word}" vao kho tu kho.`);
  }
}
